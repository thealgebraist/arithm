...
CONTEXT_SIZE = 6 
EMBED_DIM = 4   
HIDDEN_DIM = 32 # Requested "32 hidden" (assuming width 32, depth 3, based on previous "try with... 3 hidden")
NUM_HIDDEN_LAYERS = 3
BATCH_SIZE = 4096 
LEARNING_RATE = 0.005 
...
class Compressor32Deep(nn.Module):
    def __init__(self):
        super().__init__()
        self.embedding = nn.Embedding(256, EMBED_DIM)
        input_dim = CONTEXT_SIZE * EMBED_DIM
        
        # Deep Architecture: 32x32x32 (3 Hidden Layers)
        # Assuming the user means "32 width" for the deep structure requested previously
        self.fc1 = nn.Linear(input_dim, HIDDEN_DIM)
        self.fc2 = nn.Linear(HIDDEN_DIM, HIDDEN_DIM)
        self.fc3 = nn.Linear(HIDDEN_DIM, HIDDEN_DIM)
        self.fc_out = nn.Linear(HIDDEN_DIM, 256)
...
def train():
    ...
    model = Compressor32Deep().to(device)
    optimizer = optim.RAdam(model.parameters(), lr=LEARNING_RATE)
    criterion = nn.CrossEntropyLoss()
    
    print(f"Model: Embed->32->32->32->Out. Params: {sum(p.numel() for p in model.parameters())}")
    ...
    print("="*60)
    print(f"FINAL RESULT (Deep 32x32x32, 10m)")
