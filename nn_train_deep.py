...
def train():
    ...
    model = CompressorDeep().to(device)
    optimizer = optim.RAdam(model.parameters(), lr=LEARNING_RATE)
    criterion = nn.CrossEntropyLoss()
    
    print(f"Model: Embed->64->64->64->Out. Params: {sum(p.numel() for p in model.parameters())}")
    ...
    print("="*60)
    print(f"FINAL RESULT (Deep 64x64x64, 10m)")
